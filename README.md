...
- `ADB targets`: 기본값은 `192.168.0.20:35473`. 여러 폰은 한 줄에 하나씩 `IP:포트`를 입력합니다. 별명은 `거실폰=192.168.0.20:35473`처럼 설정할 수 있습니다.
- `Interval`: 5~60초, 기본값은 `20`초

중지하려면 웹 화면에서 `Stop`을 누릅니다.

`Start` 또는 `Reconnect`를 누르면 기기 목록, 별명, interval이 `tikflow-config.json`에 저장되어 서버 재시작이나 노트북 재부팅 후에도 유지됩니다.
